...
// : V если для сотрудников то акции не действуют
// TODO: активные акции
////////--------| Constructors |----------------------------------------------------------
...
Promotion.prototype.deleteNotAvailablePresent = function () {
    var ch = findAllProp( 'Promotion_id', this.ID, PromotionSubjects.list )
        , present = findAllProp( 'PresentOrChecking', true, ch[1] )[1][0]
        , i, ii
    ;
    ...
    for ( i in present.Price_id ) {
        ii = present.Price_id[i];
        if ( Product.list[ii].notAvailable || Product.list[ii].inStoplist ) {
            delete present.Price_id[i];
        }
    }
};
