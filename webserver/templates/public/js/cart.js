...
Product.prototype.makeCatalogElement = function () {
    this.catalogElement =
        '<li data-product data-not_available style="position: relative" data-id="' + this.Price_id + '" data-hash="' + this.ProductHash
        + '"><a><img onclick="" src="../../public/img/info.png" class="description_img" style="">' + this.PriceName + ' ' + this.Price + 'р.</a></li>'
};

$( document ).on( 'click', '[data-product]', function () {
    console.log( 'LI' );
    if ( !this.classList.contains( 'stop_list_product' ) ) {
        Product.setCountCart( this.dataset.id, this.dataset.id, +1 );
...
$( document ).on( 'click', '[data-product] img', function () {
    if ( !this.parentNode.parentNode.classList.contains( 'stop_list_product' ) ) {
        Product.list[this.parentNode.parentNode.dataset.id].showDescription()
    }
    return false;
} );

// запускается после получения всех доступных на точке продуктов.
Product.notAvailable = function () {
    var noAv = $( '#products_cat [data-not_available]' )
    ;
    noAv.each( function ( _, item ) {
        Product.list[item.dataset.id].notAvailable = true;
    } );
    noAv.remove();
};
...
Product.prototype.makeBtnPlusMinusElement = function ( id, val ) {
    ...
    return '<button onclick="Product.setCountCart('
        + this.Price_id + ', ' + id + ' , -1, \'minus\');Promotion.runAll();" class="minus">-</button>' + '<input onkeyup="' +
        'if (+this.value > LIMIT_IN_CART){this.value = LIMIT_IN_CART} ' +
        'Product.setCountCart(' + this.Price_id + ', ' + id + ', this.value, \'input\'); Promotion.runAll();" class="input'
        + this.Price_id + '_' + id + ' limit500 number" type="text" placeholder="0" value="' + val + '">' + '<button onclick="Product.setCountCart('
        + this.Price_id + ', ' + id + ', +1, \'plus\'); Promotion.runAll();" class="plus">+</button>';
};
...
$( document ).ready( function () {
    $( '#product_search' ).keyup( function ( e ) {
        ...
        var $Items = $( ".tab-pane .product_group li a:not(.part)" )
            , $input = $( this ), inputContent = $input.val().toLowerCase();
