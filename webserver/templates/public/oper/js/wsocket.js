...
function newWS() {
    ...
    ws.onmessage = function (msg) {
        ...
        case "ordliahis " : if (WSdata==="EOF") return;
            addElementToArrayHistory(WSdata); break;

        case "persichis" :if (WSdata==="EOF") return;
            addPersToArrayCourierHistory(WSdata); break;

        case "persimhis" :if (WSdata==="EOF") return;
            addPersToArrayManagerHistory(WSdata); break;

        case "statgshis" :  if (WSdata==="EOF") return;
            addFirstStatusToArrayHistory(WSdata); break;  // получить конкретный статус

        case "statglhis" :if (WSdata==="EOF") return;
            addLastStatusToArrayHistory(WSdata); break;  // получить последний статус

        case "useridhis":if (WSdata==="EOF") return;
            addUserToArrayHistory(WSdata);break;   ///----Читать клиента по идентификатору заказа

        case "userid" :  if (WSdata==="EOF") return;
                    addUserToArray(WSdata);break;   ///----Читать клиента по идентификатору заказа
        case "userph2" :
        case "userph" :  if (WSdata==="EOF") return;
                            addUserToTel(WSdata); break;      /// по номеру телефона во врем структуру
        case "usercr" :  console.log("Создан клиент - " + WSdata);break;   //создать клиента
        case "ordlia" :  if (WSdata==="EOF") return;
                            addElementToArray(WSdata); break;   //----Считать все элементы заказа
        case "ordlid" :   break;                                 //элемент заказов по id idi //----Считать один элемент заказа
        case "ordlgc" :   break;                                 //получить число элементов заказа
        case "ordlcr" :   console.log("Создан элемент заказа - " + WSdata); break; //создать элемент заказа

        case "perscr" :   console.log("Создан персонал - " + WSdata);break;   //создать персонал
        case "persim" : console.log("Создан персонал - " + WSdata);//break;
        if (WSdata==="EOF") return;
                            addPersToArrayManager(WSdata); break;
        case "persic" : if (WSdata==="EOF") return;
                            addPersToArrayCourier(WSdata); break;
        case "persir" :   break;                                //получить персонал по роли и ид заказа
        case "persid" :   console.log("Создан статус - " + WSdata); break;                                 //получить персонал по ид заказа"
        case "statcr" :   console.log("Создан статус - " + WSdata); break;   //создать статус
        case "statcc" :   console.log("Создан статус - " + WSdata); break;    //создать статус отменен
        case "statgs" :  if (WSdata==="EOF") return;
                            addFirstStatusToArray(WSdata); break;  // получить конкретный статус
        case "statgl" :  if (WSdata==="EOF") return;
                            addLastStatusToArray(WSdata); break;  // получить последний статус

        case "payrid" :     addPaymentToArray(WSdata);    break;     // ----Читаем все оплаты одного заказа
        case "paycid" :    console.log("Создана оплата - " + WSdata);
                            break;
        case "product":    addProduct(WSdata);   break;
        case "productOrg":    addProductOrg(WSdata);   break;
        case "orgcity":   console.log("Создана город - " + WSdata); addOrgAddressToOrg(WSdata); break;
        case "delivzone":   console.log("Получена зона - " + WSdata); setDeliveryZonePage(WSdata); break;
        ...
        case "clientInfoCr":   console.log("Получена clientInfoCr - " + WSdata); //setSessionTabel(WSdata);
        break;
        case "clientInfoUp":   console.log("Получена clientInfoUp - " + WSdata); //setSessionTabel(WSdata);
        break;
        case "clientInfoRd":
            if (WSdata==="EOF") return;
        ...
        case "clientAddressRd":
            ...
            addUserToTel(WSdata);
            break;
        case "clientAddressCr":   console.log("Получена clientAddressCr - " + WSdata); //setSessionTabel(WSdata);
        break;

        case "status" :  console.log(WSdata); break;
        case "Promo":
            console.log("Promo"+WSdata);
            if (WSdata==="EOF") { Promotion._getAll(); return;}
    ...
    if (WSerror=="02") {
        ...
        switch (data.Table){
          ...
          //  case "OrderCustomer": getorderuser(data.Values[0]);      break;
            case "OrderList":
                setTimeout( getOrderElemAll,8000, data.Values[0]);      break;
            case "OrderPayments":
                   break;
            case "OrderPersonal":
                setTimeout( getOrderPersonalCourier,8000,data.Values[0]);
                setTimeout( getOrderPersonalOperator,8000,data.Values[0]);     break; //id idi userhash hashrole
            case "OrderStatus":
...
function newClientAddress(order_id,id_adr) {
    ...
    ws.send('{"Table":"ClientInfo","TypeParameter":"CreateAddress","ID_msg":"clientAddressCr"}' +
        '{"Phone":"' + clAddress.Phone + '","Order_id":' + order_id + ', "ID":'+id_adr+', ' +
...
function getOrgbyCity(){    ws.send('{"Table":"GetPoint","Values":["Курган"],"ID_msg":"getorg"}');
}
...
//):('</td>  <td>Неверная организация</td>'))+
    ...
    $(document).ready(function(){
        ...
        else {$('#tr'+data1.ID).replaceWith(orderStr);
...
function addProduct(data) {
    ...
    $(document).ready(function () {  if (!(Product.list[data1.Price_id]))Cart.addProduct( products[data1.Price_id] );   });
...
//):('</td>  <td>Неверная организация</td>'))+
    ...
    $(document).ready(function(){
        ...
        else {$('#tr'+data1.ID+'_his').replaceWith(orderStr);
