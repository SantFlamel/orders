 ...
 */
function funcSuccessTimer(data) {
 //   console.log("arrTimer");
    try {        arrTimer = JSON.parse(data);
        console.log(arrTimer);
    }catch(err){ return
    }
   ...
   // strOrderHTML = '';
    ...
    for(var i = 0; i< arrTimer.length;i++) {
        var id=arrTimer[i].ID_order;
        var idi=arrTimer[i].ID_order_item;
        var name=arrTimer[i].Name;
        time3=timeMinus(timePlus(sliceTime(arrTimer[i].Timer_begin),time_second2 (arrTimer[i].Timer_len)),getTime1(),0 );

        valueNow= timeMinus(getTime1(),sliceTime(arrTimer[i].Timer_begin),2);
        valueMax=arrTimer[i].Timer_len;
        idBar= '#idBar'+ id  +'-'+ idi;
        console.log(idBar);
        if ($(idBar).length!=0) break;

        idBar=idBar.slice(1);
         timerbar= '<li>'+
         '<div class="progress-bar" role="progressbar" aria-valuenow=" '+ valueNow +' " aria-valuemin="0" '+
            'aria-valuemax=" '+valueMax +' " style="width: '+(valueNow/valueMax*100)+'%;"'+ ' id="'+ idBar+'" '+ ' >'+
         '<div class="time-left" data-role="timer-down-ring" label="'+name+'" label2="'+id+'" label3="'+idi+'">'+ time3+ ' </div>' +
         '<div class="pull-left">#' + id  +'-'+ idi+ ' </div>'+
         '<div>'+ name + '</div>'+
             '</div> '+
             '</li>';
       //  console.log(timerbar);

        if (time3[0]=='-' )addTimerFinishDiv(id, idi, name);
        else $('#ulprogress').append(timerbar);
       // $ ('#idBar').function(){downTimer2(down_ring)};
        /*        if (time3[0]=='-' ) {
                    addTimerFinishDiv(id, idi, name);
                    delTimerProgressbar(my_timer);
            console.log('#' + id  +'-'+ idi+ ' - добавлено оповещение')
            buttons(1, 0, 0);
        }*/
    }
    ...
    $.each(ring, function (key3, down_ring) { downTimer2(down_ring);});
    $('b').html($("#ulprogress").children().length);

 //   $(".timer_table").html(strOrderHTML);
  //  startTimer();
 //   console.log(strOrderHTML);
}


function ajaxGetDataTimer(){//получает таймеры
    var admin = "admin";
    //Вызываем ajax
    $.ajax({
        url:     "/gettimers", //То куда отправляем
        type:    "GET", //Тип передачи данных POST, GET
        data:    "",
        dataType:"text",
        success: funcSuccessTimer //Когда получен ответ выполняем нашу функцию
    });
}
/*
$(document).ready(function(){
    //обращаемся к элементу с id load, bind ждет действие которое мы указали первым параметром
    ajaxGetDataTimer();
    setInterval(ajaxGetDataTimer,10000);
});
*/
